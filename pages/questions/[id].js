  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (resAnswer) {
          resQuestion.answer = resAnswer;
          for (let i = 0; i < resQuestion.answer.length; i++) {
            let resReply = await getReply(resQuestion.answer[i].answer_id);
            console.log("resReply", resReply);

            if (resReply) resQuestion.answer[i].reply = resReply;
            else resQuestion.answer[i].reply = [];
          }
        } else {
          resQuestion.answer = [];
        }
          ...
          <div className="space-y-4 mx-10">
            <div className="flex flex-col mt-4 border-b border-gray-300">
              ...
              <p className="text-sm text-gray-500 font-medium hover:text-gray-700 my-2">
                ...
                risk_ticks = (price - sl) / symbol.trade_tick_size size = risk_capital / (risk_ticks * symbol.trade_tick_value)`}
              </p>
              <img
